...
var formatNumber = function(content, numberFormat, options) {
    ...
    if (typeof numberFormat === "undefined" || numberFormat === null)
        ...
        } else if (typeof numberFormat.exponential !== "undefined") {
            formattedNumber = content.toExponential(numberFormat.exponential);
        } else {
...
Table.prototype.getTable = function() {
    ...
    if (this.options.style !== "ascii" && this.options.style !== "csv" ) {
        ...
        if (this.options.noFloat)
            ...
            if (this.options.placement)
                rows.push(`\\begin{table}[${this.options.placement}]`);
            else
    ...
    if (this.options.style !== "ascii" && this.options.style !== "csv" && this.headerOptions.latex) {
        rows.push(this.headerOptions.latex)
    } else {
        if (this.header.trim())
    ...
    if (this.options.style !== "ascii" && this.options.style !== "csv" ) {
        ...
        if (this.options.caption !== "") {
            let escapedCaption = this.options.caption;
            if (this.options.escapeCaptions === true)
                escapedCaption = this.options.caption.replace(/([^\\])_/g, "$1\\_");
